...
const parseFloatFromObjectCopy = (objCopy) => {
  ...
  isNaN(ratio) ? objCopy.ParsedRatioToStandard = 0 : objCopy.ParsedRatioToStandard = ratio
  isNaN(quant) ? objCopy.ParsedQuantification = 0 : objCopy.ParsedQuantification = quant
...
const processOutliersFromFileData = (fileArray, outlierList) => {
  if (outlierList.length === 0 ){
    return {filteredFile: fileArray, outlier2DListOfRowGroupedByReplicate: [[]]}
  }
  const arrOfOutliersToFilter = []
  const outlier2DListOfRowGroupedByReplicate = []

  outlierList.forEach((replicateNameArr) => {

    // for each replicate name array, we will make a similar array with full samples
    const fullSampleToMirrorReplicateArr = []

    replicateNameArr.forEach((sampleReplicateName) => {

      // get list of every sample object with given replicate name
      const fullSampleByReplicate = [...fileArray].filter((row) => row.Replicate === sampleReplicateName)

      // I'd prefer concat so as not to have to flatten, but javascript is not complying :P
      arrOfOutliersToFilter.push(fullSampleByReplicate)
 ...
 const filteredFile = [...fileArray].filter((row) => !arrOfOutliersToFilter.flat(Infinity).includes(row))
...
export function quantMapper (fileDataJson, replicateNum, userOutlierSamples) {
    ...
    const parsedFileDataJson = [...fileDataJson].map((row) => {
    ...
    const {filteredFile,  outlier2DListOfRowGroupedByReplicate }= processOutliersFromFileData(parsedFileDataJson, userOutlierSamples);
    ...
    if (outlier2DListOfRowGroupedByReplicate.length !== 0 ) {
      ...
      for (
        let iterator = 0;
        iterator < outlierSamples.length;
        iterator++
      ) {
        //iterate thru 2D arr
        ...
        if (outlierSamples[iterator].length < 2) {
          //if only one obj
          const singletonListObj = outlierSamples[iterator]; //set a variable for that object for readability
          const singletonObjMapped = singletonListObj.map((singleObj) => {
            singleObj.RatioAvg =
            singleObj.ParsedRatioToStandard;
            singleObj.QuantAvg =
            singleObj.ParsedQuantification;
          ...
          const listOfOutliers = outlierSamples[iterator]; //set an obj
  ...
  return fileDataJsonArray.concat(tempOutlierList.flat(Infinity));
